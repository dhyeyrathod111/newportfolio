import React, { useEffect } from 'react';
import OtherPageHeader from '../components/OtherPageHeader';
import OtherPageBanner from '../components/OtherPageBanner';
import ServicesBeforfooter from '../components/ServicesBeforfooter';
import EcommerceCom from '../components/EcommerceCom';
import Footer from '../components/Footer';

function EcommerceDeveopment() {
    useEffect(() => window.scrollTo(0, 0));
    return (
        <React.Fragment>
            <OtherPageHeader />
            <div className="main">
                <OtherPageBanner pagename="E-Commerce Website" />
                <EcommerceCom />
                <ServicesBeforfooter />
                <Footer />
            </div>
        </React.Fragment>
    )
}

export default EcommerceDeveopment
